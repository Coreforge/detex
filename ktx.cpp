...
DETEX_API intptr_t detexLoadKTXFileWithMipmaps_memory(const unsigned char* bytes, int max_mipmaps, detexTexture*** textures_out, int* nu_levels_out)
{
	...
	int header[16];
	memcpy(header, bytes, 64);
	bytes += 64;
	copied += 64;
	if (memcmp(header, ktx_id, 12) != 0) {
		// KTX signature not found.
		detexSetErrorMessage("detexLoadKTXFileWithMipmaps_memory: Couldn't find KTX signature");
		return -1;
	}
	int wrong_endian = 0;
	if (header[3] == 0x01020304) {
	...
	const detexTextureFileInfo* info = detexLookupKTXFileInfo(glInternalFormat, glFormat, glType);
	if (info == NULL) {
		detexSetErrorMessage("detexLoadKTXFileWithMipmaps_memory: Unsupported format in .ktx file "
			"(glInternalFormat = 0x%04X)", glInternalFormat);
		return false;
	}
	int bytes_per_block;
	...
	if (header[15] > 0) {
		// Skip metadata.
		//uint8_t* metadata = (unsigned char*)malloc(header[15]);
		//memcpy(metadata, bytes, header[15]);
		bytes += header[15];
		copied += header[15];
		//free(metadata);
	}
	...
	for (int i = 0; i < nu_mipmaps; i++) {
		uint32_t image_size_buffer[1];
		memcpy(image_size_buffer, bytes, 4);
		bytes += 4;
		copied += 4;
		if (wrong_endian) {
			uint8_t* image_size_bytep = (uint8_t*)&image_size_buffer[0];
		...
		if (image_size != n * bytes_per_block) {
			for (int j = 0; j < i; j++)
				free(textures[j]);
			free(textures);
			detexSetErrorMessage("detexLoadKTXFileWithMipmaps_memory: Error loading file: "
				"Image size field of mipmap level %d does not match (%d vs %d)",
...
//Carl Kenner - Modify Detex to load DDS or KTX files stored in memory
DETEX_API intptr_t detexLoadFileWithMipmaps_memory(const unsigned char* bytes, int max_mipmaps, detexTexture*** textures_out, int* nu_levels_out)
{
	if (bytes == NULL) {
		detexSetErrorMessage("detexLoadFileWithMipmaps_memory: bytes is NULL");
		return -1;
	}
	if (memcmp(bytes, ktx_id, 12) == 0)
		return detexLoadKTXFileWithMipmaps_memory(bytes, max_mipmaps, textures_out, nu_levels_out);
	else
		return detexLoadDDSFileWithMipmaps_memory((char*)bytes, max_mipmaps, textures_out, nu_levels_out);
}
